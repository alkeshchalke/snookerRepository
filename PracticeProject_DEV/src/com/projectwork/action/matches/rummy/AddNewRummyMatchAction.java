...
public class AddNewRummyMatchAction extends ActionSupport implements TestProjectConstantsIfc, ServletRequestAware
{
    /**
     * 
     */
    private static final long serialVersionUID = 8346471015394767451L;
    ...
    public String execute() throws Exception
    {
        ...
        String secondMatchNo = matchesServiceImplObj.getNextMatchSequenceNumber(businessDate);

        boolean isSecondRummyMatchRecordInserted = matchesServiceImplObj.addNewMatchRecord(secondMatchNo, businessDate,
                employeeID, frameType, tableNo, playingCustomers, paymentAmount);
        ...
        boolean isNewCustomerSecondMatchRecordInserted = true;
        ...
        if (!customerEntryNameMap.isEmpty())
        {
            for (Map.Entry<String, String> pair : customerEntryNameMap.entrySet())
            {
                ...
                isNewCustomerSecondMatchRecordInserted = customerMatchesServiceImplObj.addNewCustomerMatchRecord(
                        currentPlayerEntryNo, secondMatchNo, businessDate, currentPlayer, frameType);

                if (!(isNewCustomerMatchRecordInserted && isNewCustomerSecondMatchRecordInserted))
                {
                    break;
                }
            }
        ...
        if (isNewMatchRecordInserted && isSecondRummyMatchRecordInserted && isCurrentlyPlayingCustomerStatusUpdated
                && isNewCustomerMatchRecordInserted)
